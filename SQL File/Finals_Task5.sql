...
SELECT P_CODE, P_DESCRIPT, P_PRICE, P_DISCOUNT, P_INDATE, V_CODE
FROM Products
...
-- 3
CREATE VIEW View_TotalPrice_By_SelectedVendors AS
SELECT p.P_CODE, p.P_DESCRIPT, p.P_ONHAND, p.P_PRICE, p.V_CODE, v.V_NAME, (p.P_ONHAND * p.P_PRICE) AS TOTAL_PRICE
FROM Products p
JOIN vendors v ON p.V_CODE = v.V_CODE
WHERE p.V_CODE IN (21344, 23119, 24288);

SELECT * FROM View_TotalPrice_By_SelectedVendors;

-- 4
DELIMITER $$
CREATE PROCEDURE Update_Vendor_Name(IN currentName VARCHAR(30))
BEGIN
UPDATE vendors
SET V_NAME = 'Bryson and Co'
WHERE V_NAME = currentName;
END $$ DELIMITER ;
...
DELIMITER $$
CREATE PROCEDURE GetProductsByVendorCodeAndState(
    IN in_vcode INT,
